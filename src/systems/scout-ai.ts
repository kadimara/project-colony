...
function nearestVisibleEnemy(
  ...
  const losCheck = (tx: number, ty: number) =>
    hasLineOfSight(x, y, tx, ty, (wx, wy) => isWall(state, wx, wy));
  let best: Enemy | null = null,
    bestDist = Infinity;
  for (const en of state.enemies) {
    if (en.hp <= 0) continue;
    const d = Math.hypot(en.tileX - x, en.tileY - y);
    if (d <= radius && d < bestDist && losCheck(en.tileX, en.tileY)) {
      best = en;
      bestDist = d;
    }
  }
...
function pickExploreTarget(
  ...
  for (let tries = 0; tries < 10; tries++) {
    ...
    const dist =
      SCOUT_EXPLORE_MIN_DIST +
      Math.random() * (SCOUT_EXPLORE_MAX_DIST - SCOUT_EXPLORE_MIN_DIST);
...
const handleAttackFlag: BTNode<ScoutCtx> = action(
  ({ state, colonist, now }) => {
...
const handleEnemySighted: BTNode<ScoutCtx> = action(
  ({ state, colonist, now }) => {
    if (colonist.scentActive && colonist.scentType === 'alarm') return;
    const sighted = nearestVisibleEnemy(
      state,
      colonist.tileX,
      colonist.tileY,
      SCOUT_VISION_RADIUS,
...
// not one tick late
const handleScentUpdate: BTNode<ScoutCtx> = action(
  ({ state, colonist, now }) => {
...
const scoutingBranch: BTNode<ScoutCtx> = action((ctx) => {
  ...
  if (
    ...
    (!foodAt(state, colonist.forageTarget.x, colonist.forageTarget.y) ||
      colonist.path.length === 0)
  ) {
    colonist.forageTarget = null;
  }
  if (!colonist.forageTarget) {
    const pull = nearestFoodTo(
      state,
      colonist.tileX,
      colonist.tileY,
      COLONIST_FORAGE_RADIUS,
